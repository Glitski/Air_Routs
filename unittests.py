...
def test(exe, tdir, sdir, use_wsl):
    test_files = list(map(lambda n: n[:-8], [file for file in os.listdir(tdir) if file.endswith(".routes")]))
    solution_files = list(map(lambda n: n[:-8], [file for file in os.listdir(sdir) if file.endswith(".bbones")]))
    ...
    if use_wsl: 
        try:
            b_path = subprocess.check_output([wsl, "wslpath", "-a", f"'{exe}'"], shell=True).decode().strip()
            t_path = subprocess.check_output([wsl, "wslpath", "-a", f"'{tdir}'"], shell=True).decode().strip()
            s_path = subprocess.check_output([wsl, "wslpath", "-a", f"'{sdir}'"], shell=True).decode().strip()
    ...
    for file in test_files:
        ...
        file_path = f"{t_path}/{file}.routes"
        if os.path.isfile(f"{t_path}/{file}.bbones"):
            os.remove(f"{t_path}/{file}.bbones")
        error = ""
        try:
            if use_wsl:
    ...
    with open("results.txt", "w") as save:
        for file in tested:
            ...
            try:
                if use_wsl:
                    subprocess.check_output([wsl, "diff", f"{t_path}/{file}.bbones", f"{s_path}/{file}.bbones"],
                                            shell=True,
                                            stderr=subprocess.PIPE)
                else:
                    subprocess.check_output(["diff", f"{t_path}/{file}.bbones", f"{s_path}/{file}.bbones"],
                                            shell=True,
                                            stderr=subprocess.PIPE)
            except subprocess.CalledProcessError as e:
                save.write(f"{file}.routes:")
